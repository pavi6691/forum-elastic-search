...
- [X] Endpoint problems:
  - [X] PUT `update`:
    - [X] Requires all of these: `externalDataSource`, `externalGuid`, `externalItemGuid`, `type` although all of them
          should be the same as previous `entryGuid` and should not be changeable.
    - [X] If updating a `guid` that is a previous version (not the latest) and with the correct `created` timestamp as
